...
bool VirtualUI::TakeScreenshot(HWND Wnd, std::wstring FileName) {
    ...
    this->TakeScreenshot((long)Wnd, fileName, &ret);
...
HRESULT __stdcall VirtualUISink::Invoke(DISPID DispID, REFIID IID, LCID LocaleID, WORD Flags, DISPPARAMS *Params, VARIANT *VarResult, EXCEPINFO *ExcepInfo, UINT *ArgErr) {
    if (DispID == 101) {
        BSTR directory = GetAsBSTR(Params, 1);
        VARIANT_BOOL handled = GetAsBoolean(Params, 0);
        OnGetUploadDir(&directory, &handled);
        SetBSTR(Params, 1, directory);
        SetVariantBool(Params, 0, handled);
    }
    if (DispID == 102) {
        long Width = GetAsInteger(Params, 2);
        long Height = GetAsInteger(Params, 1);
        VARIANT_BOOL ResizeMaximized = GetAsVariantBool(Params, 0);
        OnBrowserResize(&Width, &Height, &ResizeMaximized);
        SetVariantBool(Params, 0, ResizeMaximized);
        SetInteger(Params, 1, Height);
        SetInteger(Params, 2, Width);
    }
    if (DispID == 103) {
        OnClose();
    }
...
int VirtualUISink::GetAsInteger(DISPPARAMS* args, int index) {
	VARIANT    var;
	VariantInit(&var);
	VariantClear(&var);
	var = (args->rgvarg)[index];

	int res = 0;
    if (V_VT(&var) == VT_I4)
        res = V_I4(&var);
    if (V_VT(&var) == (VT_BYREF | VT_I4))
        res = *var.pintVal;
    return res;
}

void VirtualUISink::SetInteger(DISPPARAMS* args, int index, int value) {
    VARIANT varValue;
    VariantInit(&varValue);
    V_VT(&varValue) = VT_I4;
    V_I4(&varValue) = value;
    (args->rgvarg)[index] = varValue;
}

bool VirtualUISink::GetAsBoolean(DISPPARAMS* args, int index) {
    VARIANT    var;
    VariantInit(&var);
    VariantClear(&var);
    var = (args->rgvarg)[index];
    ...
    if (V_VT(&var) == VT_BOOL)
        res = V_BOOL(&var) != 0;
    if (V_VT(&var) == (VT_BYREF | VT_BOOL))
        res = *var.pboolVal != 0;
    return res;
}

void VirtualUISink::SetBoolean(DISPPARAMS* args, int index, bool value) {
    VARIANT varValue;
    VariantInit(&varValue);
    V_VT(&varValue) = VT_BOOL;
    V_BOOL(&varValue) = (value ? -1 : 0);
    (args->rgvarg)[index] = varValue;
}
...
void VirtualUISink::SetVariantBool(DISPPARAMS* args, int index, VARIANT_BOOL value) {
    VARIANT varValue;
    VariantInit(&varValue);
    V_VT(&varValue) = VT_BOOL;
    V_BOOL(&varValue) = value;
    (args->rgvarg)[index] = varValue;
}

std::wstring VirtualUISink::GetAsString(DISPPARAMS* args, int index) {
    VARIANT    var;
    VariantInit(&var);
    VariantClear(&var);
    var = (args->rgvarg)[index];
    ...
    if (V_VT(&var) == VT_BSTR)
        res = V_BSTR(&var);
    if (V_VT(&var) == (VT_BYREF | VT_BSTR))
        res = *var.bstrVal;
    return res;
}

void VirtualUISink::SetString(DISPPARAMS* args, int index, std::wstring value) {
...
void VirtualUISink::SetBSTR(DISPPARAMS* args, int index, BSTR value) {
    VARIANT varValue;
    VariantInit(&varValue);
    V_VT(&varValue) = VT_BSTR;
    V_BSTR(&varValue) = value;
    (args->rgvarg)[index] = varValue;
}
...
IDispatch* VirtualUISink::GetAsInterface(DISPPARAMS* args, int index) {
	...
	if (V_VT(&var) == VT_DISPATCH)
		return V_DISPATCH(&var);
	if (V_VT(&var) == (VT_BYREF | VT_DISPATCH))
		return var.pdispVal;
	return NULL;
}
...
HRESULT __stdcall VirtualUISink::OnGetUploadDir(BSTR *Directory, VARIANT_BOOL* Handled) {
    if (m_VirtualUI->OnGetUploadDir) {
        ...
        //TODO: SysFreeString?
        *Handled = (h ? -1 : 0);
        *Directory = SysAllocString(dir.c_str());
    }
    return S_OK;
}
...
int JSObjectSink::GetAsInteger(DISPPARAMS* args, int index) {
	VARIANT    var;
	VariantInit(&var);
	VariantClear(&var);
	var = (args->rgvarg)[index];
...
void JSObjectSink::SetInteger(DISPPARAMS* args, int index, int value) {
    VARIANT varValue;
    VariantInit(&varValue);
    V_VT(&varValue) = VT_I4;
    V_I4(&varValue) = value;
...
bool JSObjectSink::GetAsBoolean(DISPPARAMS* args, int index) {
    VARIANT    var;
    VariantInit(&var);
    VariantClear(&var);
    var = (args->rgvarg)[index];
...
void JSObjectSink::SetBoolean(DISPPARAMS* args, int index, bool value) {
    VARIANT varValue;
    VariantInit(&varValue);
    V_VT(&varValue) = VT_BOOL;
    V_BOOL(&varValue) = (value ? -1 : 0);
...
VARIANT_BOOL JSObjectSink::GetAsVariantBool(DISPPARAMS* args, int index) {
    VARIANT    var;
    VariantInit(&var);
    VariantClear(&var);
    var = (args->rgvarg)[index];
...
void JSObjectSink::SetVariantBool(DISPPARAMS* args, int index, VARIANT_BOOL value) {
    VARIANT varValue;
    VariantInit(&varValue);
    V_VT(&varValue) = VT_BOOL;
    V_BOOL(&varValue) = value;
...
std::wstring JSObjectSink::GetAsString(DISPPARAMS* args, int index) {
    VARIANT    var;
    VariantInit(&var);
    VariantClear(&var);
    var = (args->rgvarg)[index];
...
void JSObjectSink::SetString(DISPPARAMS* args, int index, std::wstring value) {
    BSTR bValue = SysAllocString(value.c_str());
    VARIANT varValue;
    VariantInit(&varValue);
    V_VT(&varValue) = VT_BSTR;
...
BSTR JSObjectSink::GetAsBSTR(DISPPARAMS* args, int index) {
	VARIANT    var;
    VariantInit(&var);
    VariantClear(&var);
    var = (args->rgvarg)[index];
...
void JSObjectSink::SetBSTR(DISPPARAMS* args, int index, BSTR value) {
    VARIANT varValue;
    VariantInit(&varValue);
    V_VT(&varValue) = VT_BSTR;
    V_BSTR(&varValue) = value;
