...
begin
  ...
  begin
    ...
    FIsValidSDK := ValidateInterface();
  end;
end;

destructor TPrinter.Destroy;
begin
  if Assigned(GRemotePrinter) and (Self = GRemotePrinter) then
    GRemotePrinter := nil;
  inherited Destroy;
end;

// Internal validation between this SDK with the running dll IDispatch.
function TPrinter.ValidateInterface(): Boolean;
var
  res,tCount: Integer;
  tInfo: ITypeInfo;
  function ValidateMethod(const MethodName: string; const Args: array of string): Boolean;
  var
    Names : array [0..0] of PWideChar;
    NameDispIds : array [0..0] of TDispID;
    rgBStrList: TBStrList;
    I,R,cmaxNames, cNames: Integer;
  begin
    Result := False;
    Names[0] := StringToOleStr(MethodName);
    R := FIPrinter.GetIDsOfNames (GUID_NULL, @Names, 1, LOCALE_SYSTEM_DEFAULT,@NameDispIds);
    if ( R <> S_OK) then Exit;
    R := tInfo.GetNames(NameDispIds[0], @rgBStrList, cmaxNames, cNames);
    if ( R <> S_OK) then Exit;
    if (cNames <> Length(Args)+1) then Exit;
    for I := Low(Args) to High(Args) do begin
      if (OleStrToString(rgBStrList[I+1]) <> Args[I]) then Exit;
    end;
    Result := True;
  end;
begin
  ...
  function PrintFile(PrintType: Integer; const FileName: WideString;
                      const PrinterName: WideString): WordBool; dispid 207;}
  Result := False;
  if not Assigned(FIPrinter) then Exit;
  try
    // Obtengo el ITypeInfo de IDisp
    res := FIPrinter.GetTypeInfoCount(tCount);
    if ( res <> S_OK) or (tCount < 1) then Exit;
    res := FIPrinter.GetTypeInfo(0,LOCALE_SYSTEM_DEFAULT, tInfo);
    if ( res <> S_OK) and (tInfo = nil) then Exit;

    if not ValidateMethod('BeginDoc',['PrintType','PrinterName','DocName','Encoding','DocID']) then Exit;
    if not ValidateMethod('Print',['DocID','Data']) then Exit;
    if not ValidateMethod('EndDoc',['DocID']) then Exit;
    if not ValidateMethod('Abort',['DocID']) then Exit;
    if not ValidateMethod('LastError',['ErrorCode','ErrorMessage']) then Exit;
    if not ValidateMethod('GetPrinters',['Delimiter','Printers']) then Exit;
    if not ValidateMethod('PrintFile',['PrintType','FileName','PrinterName']) then Exit;
...
begin
  if Assigned(FIPrinter) then
    if not FIsValidSdk then begin
      ErrorCode := ERROR_PROC_NOT_FOUND;
      ErrorMessage  := Format('This SDK does not support the loaded module "%s".',['Thinfinity.RemotePrinter.dll']);
      Exit;
    end else
  ...
  begin
    ...
    ErrorMessage  := Format('The specified module "%s" could not be found.',['Thinfinity.RemotePrinter.dll']);
  end else begin
    ErrorCode := 0;
...
initialization
  LoadLib;
finalization
  if Assigned(GRemotePrinter) then
